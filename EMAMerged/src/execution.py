...
def execute_long_with_oco(
    ...
    try:
        if prefer_limit_entry and limit_px is not None:
            logger.entry_submit(symbol=symbol, session=session, cid=cid, side="BUY",
        ...
        logger.entry_ack(symbol=symbol, session=session, cid=cid,
                         client_order_id=coid_parent,
                         broker_order_id=ack.get("id"), status=ack.get("status"))
    except Exception as e:
        logger.entry_reject(symbol=symbol, session=session, cid=cid,
    ...
    if not ok:
        ...
        logger.error(symbol=symbol, session=session, cid=cid, stage="ENTRY_WAIT",
    ...
    logger.entry_fill(symbol=symbol, session=session, cid=cid,
